...
`gb` is a Rust CLI tool that wraps `git branch` with a worktree-based workflow. Each branch gets its own directory instead of switching branches in-place.
...
src/
  ...
  config.rs            — Read gb.worktreeDir, gb.naming from git config
...
- **`__gb_cd:` protocol**: The binary prints `__gb_cd:/path` to stdout when it wants the shell to `cd`. The shell wrapper function (from `eval "$(gb init zsh)"`) intercepts this.
...
- `gb init <url>` — bare clone with worktree layout
- `gb init` — in-place conversion of existing repo
...
- `gb init` in-place conversion needs more testing with complex repos
- Consider adding `gb prune` to clean stale worktrees
